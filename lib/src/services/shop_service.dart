...
class ShopService {
  ...
  Future<Product?> createProduct(CreateProductRequest request) async {
    ...
    // 1. Create a clean Map of your data (Excluding images)
    final dataMap = {
      ...
      // Map the variants list to JSON-friendly list
    ...
    // 2. Prepare the Multipart Fields
    // We send the whole object as a single JSON string string under the key 'data'
  ...
  Future<Shop?> updateUserDetail(UpdateShopRequest request) async {
    final url = "$baseUrl/${request.shopId}";
    ...
    try {
      ...
      if(response.statusCode == 200 || response.statusCode == 201) {
        ...
        final errorMap = jsonDecode(response.body);
        throw Exception(errorMap['message'] ?? "Error update shop");
      }
    } catch (e) {
      throw Exception("Update shop service error: $e");
    }
  }
  ...
  Future<ApiResponse<Product>> updateProduct(int productId, UpdateProductRequest request) async {
    ...
    final dataMap = {
      ...
      'status': request.status,
      'categoryId': request.categoryId,
      'keepImageUrls': request.keepImageUrls, // Send this list directly in JSON
      'variants': request.variants.map((v) => {
        'variantId': v.variantId, // Can be null for new variants
        'variantType': v.variantType,
        'variantValue': v.variantValue,
        'price': v.price,
        'stock': v.stock,
    ...
    // 2. Prepare the Multipart Fields
    // We send the whole object as a single JSON string under the key 'data'
    final fields = <String, String>{
      'data': jsonEncode(dataMap),
    };

    print("FIELDS???: ${fields}");
    print("IMAGES????: ${request.newImages}");
  ...
  Future<Shop?> createShop(CreateShopRequest request) async {
    ...
    print("CREATE SHOP FIELDS???: ${fields}");
    ...
    try {
      ...
      print("CREATE SHOP RESPONSE???: ${response.body}");
